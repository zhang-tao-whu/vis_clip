...
class ReferringTracker_noiser(torch.nn.Module):
    def __init__(
        ...
        # init heads, shared with CTVIS, including out_norm & class & mask & reid
        self.class_embed = transformer_decoder.class_embed
        self.mask_embed = transformer_decoder.mask_embed
        self.reid_embed = transformer_decoder.reid_embed
        self.decoder_norm = transformer_decoder.decoder_norm
    ...
    def forward(self, frame_embeds, mask_features, resume=False,
                ...
                frame_embeds_no_norm=None, cur_feature=None, reid_embeds=None):
        """
        ...
        if reid_embeds is not None:
            reid_embeds = reid_embeds.permute(2, 3, 0, 1)  # t, q, b, c
        ...
        for i in range(n_frame):
            ...
            if reid_embeds is not None:
                single_frame_reid_embeds = reid_embeds[i]
            else:
                single_frame_reid_embeds = single_frame_embeds
            ...
            if i == 0 and resume is False:
                ...
                for j in range(self.num_layers):
                    if j == 0:
                        ...
                        self.last_frame_embeds = single_frame_reid_embeds[indices]
                        ret_indices.append(indices)
                        output = self.transformer_cross_attention_layers[j](
                            noised_init, single_frame_reid_embeds, single_frame_reid_embeds, single_frame_embeds_no_norm,
                            memory_mask=None,
                            memory_key_padding_mask=None,
                        ...
                        output = self.transformer_cross_attention_layers[j](
                            ms_output[-1], single_frame_reid_embeds, single_frame_reid_embeds, single_frame_embeds_no_norm,
                            memory_mask=None,
                            memory_key_padding_mask=None,
                            pos=None, query_pos=None
                        )
                ...
                for j in range(self.num_layers):
                    cur_reference = self.reid_embed(self.decoder_norm(self.last_outputs[-1]))
                    if j == 0:
                        indices, noised_init = self.noiser(
                        ...
                        ret_indices.append(indices)
                        output = self.transformer_cross_attention_layers[j](
                            noised_init, cur_reference, single_frame_reid_embeds, single_frame_embeds_no_norm,
                            memory_mask=None,
                            memory_key_padding_mask=None,
                            pos=None, query_pos=None
                        )
                        ...
                        output = self.transformer_cross_attention_layers[j](
                            ms_output[-1], cur_reference, single_frame_reid_embeds, single_frame_embeds_no_norm,
                            memory_mask=None,
                            memory_key_padding_mask=None,
                            pos=None, query_pos=None
                        )
