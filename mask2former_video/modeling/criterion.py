...
class VideoSetCriterion(nn.Module):
    ...
    def loss_contrast(self, outputs, targets, indices, num_masks, neg_num=3):
        ...
        for i in range(num_video):
            ...
            for j in range(self.frames):
                idx = i * self.frames + j
                indice = indices[idx]
                frame_masks.append(targets[idx]['masks'][indice[1]])  # (n, h, w)
                frame_ids.append(targets[idx]['ids'][indice[1]])  # (n)
        ...
        empty_weight[0] = 1
        loss_contrast = F.cross_entropy(cos_sim, target_classes, empty_weight)
    ...
    def _select_pos_neg_embeds(self, frame_masks, frame_ids, frame_embeds, refer_embeds, pos_embeds, neg_embeds, neg_num=3):
        ...
        for i in range(self.frames):
            ...
            if _neg_num != neg_num:
                id_neg = torch.cat([id_neg, id_neg[:, :neg_num - _neg_num]], dim=1)
