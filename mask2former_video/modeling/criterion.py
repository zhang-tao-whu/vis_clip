...
class VideoSetCriterion_(nn.Module):
    """This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
    ...
    def loss_labels(self, outputs, targets, indices, num_masks):
        ...
        src_logits = outputs["pred_logits"].float() # (bs, nq, t, c)

        idx = self._get_src_permutation_idx(indices)
        target_classes_o = torch.cat([t["labels"][J] for t, (_, J) in zip(targets, indices)], dim=0) # (n_idx, T)
        target_classes = torch.full(
        ...
        target_classes[idx] = target_classes_o.unsqueeze(1).repeat(1, target_classes.size(2))

        src_logits = src_logits.flatten(1, 2) # (bs, nq*t, c)
        target_classes = target_classes.flatten(1, 2) # (bs, nq*t)
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        src_idx = self._get_src_permutation_idx(indices)
        src_masks = outputs["pred_masks"] # (bs, nq, t, h, w)
        src_masks = src_masks[src_idx] # (n_matched, t, h, w)
        # Modified to handle video
        # t['masks'] (N, T, H, W)
        target_masks = torch.cat([t['masks'][i] for t, (_, i) in zip(targets, indices)]).to(src_masks)
        # (n_matched, T, H, W)
    ...
    def _select_pos_neg_embeds(self, frame_masks, frame_ids, frame_embeds, refer_embeds, pos_embeds, neg_embeds,
                               neg_num=3):
        assert len(frame_masks) == len(frame_embeds) == len(frame_ids) == self.frames
        refer_idx = self.frames // 2
        refer_id = frame_ids[refer_idx]
