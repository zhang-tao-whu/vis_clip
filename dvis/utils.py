...
class Noiser:
    ...
    def __call__(self, ref_embeds, cur_embeds, activate=False, cur_classes=None, normer=None):
        if normer is not None:
            matched_indices = self.match_embds(normer(ref_embeds), normer(cur_embeds))
        else:
