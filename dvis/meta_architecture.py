...
class DVIS_online(MinVIS):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if self.iter < self.max_iter_num // 2:
                ...
                losses = self.criterion(outputs, targets, pre_indices=pre_indices, matcher_outputs=image_outputs)
            else:
                # image_outputs, outputs, targets = self.frame_decoder_loss_reshape(
    ...
    def pre_forward_matcher(self, image_outputs, targets):
        image_outputs = {k: v for k, v in image_outputs.items() if k != "aux_outputs"}
    ...
    def pre_match(self, image_outputs, frame_embds):
        ...
        frame_embds[0] = frame_embds[0][:, frame_indices, indices]
