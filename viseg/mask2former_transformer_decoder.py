...
class MultiScaleMaskedTransformerDecoder_Prompt(nn.Module):
    ...
    def forward_video(self, x, mask_features, mask=None, track_infos=None):
        ...
        prompt_embed_1 = track_embed[:1].unsqueeze(1).repeat(n_track, 1, 1)
    ...
    def forward(self, x, mask_features, mask = None, track_infos=None):
        # x is a list of multi-scale feature
        assert len(x) == self.num_feature_levels
        if track_infos is not None:
            return self.foward_video(x, mask_features, mask = None, track_infos=track_infos)
        src = []
        pos = []
        size_list = []
